function [p,estState,res] = stateUpdateTest(p, cpt, dt)
tic
%-------------------%
% Initialize
...
lever_arm_e = R_e2b_hat' * p.imu_lever_arm;  % Lever arm in ECEF frame.
...
for j=1:length(y_rho)
    ...
    elseif p.state_mode == p.ins_mode
        ...
        %H(j+length(y_rho),13:15) = los_r*R_e2b_hat'*vectorSkewSymMat(p.imu_lever_arm);
        gyro_noise2dop(j) = H(j+length(y_rho),13:15)*(p.imu_para.gyro_noise*eye(3,3))*H(j+length(y_rho),13:15)';
        res_v(j) = y_dop(j) - los_r*(x_minus(4:6) - s_v_ecef(:,j));%-los_r*Omg_lever;
    end
...
res = y_rho - r - H_os(1:length(y_rho),16:end)*x_minus(17:end);

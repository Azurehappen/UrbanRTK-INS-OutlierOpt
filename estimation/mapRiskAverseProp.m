function [flag,x_post,P_post,b,J_out,augcost,num_iter,constraint] =...
    mapRiskAverseProp(num_constrain,y,H,P,R,J_l,x_prior,td_lambda,num_range)
...
%         % -G b -Jp <= -u * (G*b_max + Jp)
%         % => -G b + u * (G*b_max + Jp) <= Jp
%         slack_mat(ind(i),i) = -max_neg_lhs(ind(i))+Jl_diag(ind(i));
%     end
%     ieqLHS = [ieqLHS, slack_mat];
%     ieqRHS(ind) = Jl_diag(ind);
%     lowerbound = [zeros(m+length(ind),1)]; % lower bound on measurement selection vector b+s
%     upperbound = [ones(m+length(ind),1)];  % upper bound on measurement selection vector b+s
%     flag = false;
% end
if ~all(ieqLHS*b <= ieqRHS)
    % Add slack variable for soft constraint
